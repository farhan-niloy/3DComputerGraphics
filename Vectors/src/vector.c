...
vec3_t vec3_add(vec3_t a, vec3_t b, vec3_t c) {
  vec3_t result = {
      .x = a.x + b.x + c.z, .y = a.y + b.y + c.z, .z = a.z + b.z + c.z};
  return result;
}

vec3_t vec3_sub(vec3_t a, vec3_t b, vec3_t c) {
  vec3_t result = {
      .x = a.x - b.x - c.z, .y = a.y - b.y + c.z, .z = a.z - b.z - c.z};
  return result;
}
